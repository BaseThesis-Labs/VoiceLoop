...
const CustomTooltip = ({ active, payload, label, suffix = "ms" }: { active?: boolean; payload?: any[]; label?: string; suffix?: string }) => {
  if (active && payload && payload.length) {
    return (
      <div style={{
        ...
        {payload.map((entry: { color?: string; fill?: string; name?: string; dataKey?: string; value?: number }, i: number) => (
...
function CascadedPipelineDiagram() {
  ...
  const Arrow = ({ x1, y1, x2, y2, label, labelY, dashed }: { x1: number; y1: number; x2: number; y2: number; label?: string; labelY?: number; dashed?: boolean }) => (
    <g>
      <line
  ...
  // Reusable box
  const Box = ({ x, y, w, h, label, sublabel, fill = "none", stroke = "#555", textColor = "#ddd", fontSize = 12 }: { x: number; y: number; w: number; h: number; label: string; sublabel?: string; fill?: string; stroke?: string; textColor?: string; fontSize?: number }) => (
    <g>
...
function ThreeArchitecturesDiagram() {
  ...
  const SmallBox = ({ x, y, w, h, label, fill = "none", stroke = "#555", textColor = "#ccc", fontSize = 10.5 }: { x: number; y: number; w: number; h: number; label: string; fill?: string; stroke?: string; textColor?: string; fontSize?: number }) => (
    <g>
  ...
  const SmallArrow = ({ x1, y1, x2, y2, label, color = "#888" }: { x1: number; y1: number; x2: number; y2: number; label?: string; color?: string }) => (
    <g>
  ...
  // Tradeoff row
  const TradeoffRow = ({ x, y, label, cascade, s2s, half }: { x: number; y: number; label: string; cascade: string; s2s: string; half: string }) => (
    <g>
